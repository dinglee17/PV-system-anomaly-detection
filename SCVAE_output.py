...
def mean_relative_error(y_true, y_pred):
    ...
    relative_error = relative_error / \
        (np.max(y_true, axis=0)-np.min(y_true, axis=0))
...
def train_one_epoch(dataloader, model, optimizer, reg, mode):
    train_loss = 0
    for batch_idx, (data, y_data) in enumerate(dataloader):
        # 转置成T,batch_size,feature_dim,input_dim
        data = data.permute(1, 0, 3, 2).to(device)
        ...
        if mode == 1:
            loss = model.kld_loss + model.nll_loss + reg * model.smooth_loss + \
                model.nll_loss_prior + 0 * model.smooth_loss_prior  # mode1  prior
        if mode == 0:
            loss = model.kld_loss + model.nll_loss + \
                reg * model.smooth_loss  # mode0 reconstruct
        if mode == 2:
            loss = model.kld_loss + model.nll_loss + reg * model.smooth_loss + \
                model.kld_loss_predict + model.nll_loss_predict  # mode2 predict
...
def train(train_loader, test_loader, model,  optimizer, reg, mode):
    ...
    for t in range(epochs):
        ...
        if (t+1) % 500 == 0:
            ...
            ), "saved_model/2mode1_revise_CVAE_Model_SISVAE_guangfu_z128_h512_epoch%d" % (t))
...
def draw_anomaly_withMask(ra, tprt, pow_, predict, label, index, y_label1, y_label2, anomaly_mask, savefig=None):
    ...
    color = 'b' if label == 0 else 'r'
    ax.scatter(x, pow_, s=10, c=color, alpha=1, label="True")  # 绘制散点图
    ax.plot(x, pow_, c=color, lw=1.5)  # 绘制折线图
    ...
    for i in range(len(anomaly_mask)):
        if anomaly_mask[i] > 0:
            ax.scatter(x[i], pow_[i], color='',
                       marker='o', edgecolors='r', s=150)
    # for i in range(len(label)):
        # if label[i]==1:
            #ax.scatter(x[i], series[i], color='', marker='o', edgecolors='r', s=200)
    ...
    ax.set_ylabel("value(scaled)", fontsize=20)
    ax.set_title("power", fontsize=20)

    ax2.set_xlabel('time points', fontsize=20)
    # ax2.set_ylabel("value(scaled)",fontsize=20)
    ax2.set_title("radiation", fontsize=20)

    ax4.set_xlabel('time points', fontsize=20)
    # ax2.set_ylabel("value(scaled)",fontsize=20)
    ax4.set_title("temperature", fontsize=20)
...
def draw_anomaly_infor(ra, tprt, pow_, predict, label, index, y_label1="power/cap", y_label2="radiation", y_label3="temperature", datetime=None, sitenum=None, city=None, savefig=None):
    ...
    ax3.scatter(x, predict, s=10, c='b', alpha=1)  # 绘制散点图
    color = 'g'
    ax3.plot(x, predict, c=color, lw=1.5, label="simulate pow")  # 绘制折线图
    ax3.legend(loc="best")
    ...
    if datetime != None:
        ax4.annotate(datetime, (50, (np.max(tprt)/2+np.min(tprt)/2)),
                     fontsize=14, horizontalalignment='center')
    #assert sitenum!=None
    # print(type(sitenum))
    if sitenum != None:
        site_longitude = site_infor[site_infor["proj_id"].astype(
            'string') == sitenum]["longitude"].values[0]
        site_lattitude = site_infor[site_infor["proj_id"].astype(
            'string') == sitenum]["lattitude"].values[0]
        # print(site_name)

        ax4.annotate("site_num: "+str(sitenum), (50, (2*np.max(tprt) /
                                                      3+np.min(tprt)/3)), fontsize=14, horizontalalignment='center')
        ax4.annotate("longitude: "+str(round(site_longitude, 4))+" lattitude: "+str(round(site_lattitude, 4)),
                     (50, (np.max(tprt)/3+2*np.min(tprt)/3)), fontsize=14, horizontalalignment='center')
        #ax4.annotate("lattitude: "+str(site_lattitude),(50,(np.max(tprt)/4+3*np.min(tprt)/4)),fontsize=14,horizontalalignment='center')
        ax4.annotate("city: "+str(city), (50, (np.max(tprt)/5+4 *
                                               np.min(tprt)/5)), fontsize=14, horizontalalignment='center')
...
def draw_anomaly_picture(ra, tprt, pow_, predict, label, index, y_label1="power/cap", y_label2="radiation", y_label3="temperature", datetime=None, sitenum=None, city=None, savefig=None):
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dataset", type=str,
                        default='data/site_30100005_series.npz', help='data file path')
    ...
    parser.add_argument("--normal_ratio", type=int,
                        default=99.9, help="the nomal_ratio % in dataset")
    parser.add_argument("--mode", type=int, default=2,
                        help="the mode when train")
    parser.add_argument("--is_predict", type=bool,
                        default=False, help="whether predict ot not")
    parser.add_argument("--is_prior", type=bool, default=False,
                        help="whether prior reconstruct ot not")
    parser.add_argument("--whether_get_z", type=bool,
                        default=False, help="whether get hidden variable")
    ...
    # ra_series_scale,pow_series_scale,tprt_series_scale,anomaly_mask=read_single_anomaly_case("2test_shading_anomaly_data_of_10sites.csv",test=True,include_normal=True) #"smooth_intense_normal_data_of_20sites.csv"
    ...
    if len(list(date_series_simulate)) != 0:
        ...
        date_series_simulate = date_series_simulate.reshape(
            -1, seq_len*input_dim)
        print(date_series_simulate.shape)

    if len(list(site_num_series_simulate)) != 0:
        print("site_num_series_simulate not empty")
        site_num_series_simulate = site_num_series_simulate.reshape(
            -1, seq_len*input_dim)

    if len(list(ra_series_noscale_simulate)) != 0:
        print("ra_series_noscale_simulate not empty")
        ra_series_noscale_simulate = ra_series_noscale_simulate.reshape(
            -1, seq_len*input_dim)
        print(ra_series_noscale_simulate.shape)
        print(ra_series_noscale_simulate[0])

    if len(list(pow_series_noscale_simulate)) != 0:
        print("pow_series_noscale_simulate not empty")
        pow_series_noscale_simulate = pow_series_noscale_simulate.reshape(
            -1, seq_len*input_dim)
        print(pow_series_noscale_simulate.shape)
        print(pow_series_noscale_simulate[0])

    if len(list(tprt_series_noscale_simulate)) != 0:
        print("ra_series_noscale_simulate not empty")
        tprt_series_noscale_simulate = tprt_series_noscale_simulate.reshape(
            -1, seq_len*input_dim)
    ...
    if opt.test_ratio != 1:
        x_train1, x_test1, x_train2, x_test2, y_train, y_test = train_test_split(
            ra_per_day_seq, tprt_per_day_seq, pow_per_day_seq, test_size=opt.test_ratio, random_state=42)  # random_state一样，获得的结果就是一样的
        if len(list(anomaly_mask)) != 0:
    ...
    print(f"data shape：{train_chunks.shape}")
    ...
    model = Normal_VRNN(x_dim, y_dim, h_dim, z_dim, input_dim,
                        1, device=device, is_prior=False).to(device)
...
# 模型输出
    ...
    # model_path="./saved_model/revise_CVAE/20site_mode1_revise_CVAE_Model2_SISVAE_guangfu_z128_h512_lr_10-5_reg0_epoch15999"
    model_path = "./saved_model/20site_mode2_revise_CVAE_Model2_SISVAE_guangfu_z128_h512_lr_10-6_reg0_epoch29999"  # mode2
    # model_path="./saved_model/revise_CVAE/20site_mode0_revise_CVAE_Model2_SISVAE_guangfu_z128_h512_lr_10-6_reg0_epoch7999"
    # model_path="./saved_model/revise_CVAE/20site_mode1_revise_CVAE_Model2_SISVAE_guangfu_z128_h512_lr_10-4_reg0_epoch16999"

    # model_path="./saved_model/revise_CVAE/20site_mode2_removeTPRT_CVAE_Model2_guangfu_z128_h512_lr_10-6_reg0_epoch5999"
    ...
    if not is_predict:
        mus, stds, scores = reconstruct(
            model, show_dataloader, is_prior)  # mode0或mode1
    else:
    ...
    if len(list(anomaly_mask)) != 0:
        ...
        if detect_type == "spike":
            label_part = label[np.where((label == 1*72) | (label == 0))[0]]
            print(label_part.shape)
            anomal_score = anomal_score[np.where(
                (label == 1*72) | (label == 0))[0]]
        if detect_type == "no_output":
            label_part = label[np.where((label == 2*72) | (label == 0))[0]]
            anomal_score = anomal_score[np.where(
                (label == 2*72) | (label == 0))[0]]
        if detect_type == "snowy":
            label_part = label[np.where((label == 3*72) | (label == 0))[0]]
            anomal_score = anomal_score[np.where(
                (label == 3*72) | (label == 0))[0]]
        if detect_type == "cloudy":
            label_part = label[np.where((label == 4*72) | (label == 0))[0]]
            anomal_score = anomal_score[np.where(
                (label == 4*72) | (label == 0))[0]]
        if detect_type == "low_value":
            label_part = label[np.where((label == 5*72) | (label == 0))[0]]
            anomal_score = anomal_score[np.where(
                (label == 5*72) | (label == 0))[0]]
        if detect_type == "shading":
            label_part = label[np.where((label == 6*72) | (label == 0))[0]]
            anomal_score = anomal_score[np.where(
                (label == 6*72) | (label == 0))[0]]
    ...
    percentile_1 = np.percentile(
        anomal_score, nomal_ratio)  # 假设有5%的点是异常点计算98分位点
    ...
    if normal_pattern == 1:
        ...
        for i, nomal in enumerate(nomal_inst):
            ...
            if len(list(anomaly_mask)) != 0:
                ...
                print(date_)
                # print(site_num_)
                site_index = np.unique(
                    np.where(site_num_series_simulate == site_num_)[0])
                # print(site_index)
                # print(site_num_series_simulate[site_index])
                # print(date_series_simulate[site_index])
                date_index = np.unique(
                    np.where(date_series_simulate[site_index] == date_)[0])
                # print(date_index)
                print(date_series_simulate[site_index][date_index][0][0])
                pow_simulate = pow_series_noscale_simulate[site_index][date_index].reshape(
                    (72))
                # print(pow_simulate.shape)
                ...
                pow_simulate = pow_simulate*pow_std+pow_mean

                # draw_anomaly_infor(ra,tprt,pow_,pow_simulate,0,2*i,datetime=date_,sitenum=site_num_,city=City_dict[site_num_])
                draw_anomaly_picture(ra, tprt, pow_, pow_simulate, 0, 2*i,
                                     datetime=date_, sitenum=site_num_, city=City_dict[site_num_])
            else:
    ...
    for i, anomal in enumerate(anomal_inst):
        ...
        if len(list(anomaly_mask)) != 0:
            ...
            draw_anomaly_infor(ra, tprt, pow_, pow_simulate, 1, 2*i, datetime=date_, sitenum=site_num_,
                               city=City_dict[site_num_], savefig="anomal_index_"+str(anomal)+".png")
        else:
    ...
    if normal_pattern == 0:
        ...
        for i, nomal in enumerate(nomal_inst):
            ...
            if len(list(anomaly_mask)) != 0:
                ...
                draw_anomaly_infor(ra, tprt, pow_, pow_simulate, 0, 2*i,
                                   datetime=date_, sitenum=site_num_, city=City_dict[site_num_])
            else:
                draw_anomal(ra, tprt, pow_, predict_, 0, 2*i, 'pow', 'ra')
            # draw_anomal(ra,tprt,pow_,predict,0,2*i+1,'pow','ra')
