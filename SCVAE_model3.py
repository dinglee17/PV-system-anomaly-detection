...
class Normal_VRNN(nn.Module):
    def __init__(self, x_dim, label_dim, h_dim, z_dim, input_dim,bias=False, device=None, is_prior=False):
        ...
        self.dec_mean_prior = nn.Sequential(
                nn.Linear(h_dim, h_dim),
                nn.LeakyReLU(0.1),
                nn.Linear(h_dim, input_dim*label_dim),
                nn.LeakyReLU(0.1))
        
        self.dec_std_prior = nn.Sequential(
            nn.Linear(h_dim, input_dim*label_dim),
            nn.Softplus())
        
        self.dec_mean_predict = nn.Sequential(
                nn.Linear(h_dim, h_dim),
                nn.LeakyReLU(0.1),
                nn.Linear(h_dim, input_dim*label_dim),
                nn.LeakyReLU(0.1))
        
        self.dec_std_predict = nn.Sequential(
            nn.Linear(h_dim, input_dim*label_dim),
            nn.Softplus())
    ...
    def reucrrence(self, x_t, y_t, h, h2, recording=True): #prior_flag 的含义是先验推导还是后验推导,如果为1的话表明先验不使用标签，如果为0的话表明为后验，使用标签,如果为2的话采用两种模式混合的方式
        ...
        dec_mean_t_prior = self.dec_mean_prior(dec_t_prior)
        #print(dec_mean_t_prior.detach().cpu().numpy())
        dec_std_t_prior = self.dec_std_prior(dec_t_prior)
        
        dec_t_predict = self.dec_predict(torch.cat([phi_x_t,phi_z_t_predict, h2],1)) # 考虑的是xt,zt条件下的y值
        dec_mean_t_predict = self.dec_mean_predict(dec_t_predict)
        #print(dec_mean_t_predict.detach().cpu().numpy())
        dec_std_t_predict = self.dec_std_predict(dec_t_predict)
    ...
    def predict(self, X, SMC_iter = 1):
        ...
        for i in range(SMC_iter):
            ...
            for t in range(X.shape[0]):
                ...
                #phi_y_t = self.phi_y(y_t)
                # encoding
                predict_t = self.predict(torch.cat([phi_x_t, h2],1)) #单纯的预测只考虑纯x
                predict_mean_t = self.predict_mean(predict_t)
                predict_std_t = self.predict_std(predict_t)
                # sampling and reparamerization
                ...
                # decoding
                dec_t = self.dec_predict(torch.cat([phi_x_t,phi_z_t, h2],1)) # 考虑的是xt,zt条件下的y值
                dec_mean_t = self.dec_mean_predict(dec_t)
                #print(dec_mean_t.detach().cpu().numpy())
                dec_std_t = self.dec_std_predict(dec_t)
        ...
        for i in range(SMC_iter):
            ...
            for t in range(X.shape[0]):
                ...
                if(not is_prior):
                    ...
                    # decoding
                    dec_t = self.dec_prior(torch.cat([phi_x_t,phi_z_t, h],1)) # 考虑的是xt,zt条件下的y值
                    dec_mean_t = self.dec_mean_prior(dec_t)
                    #print("dec_mean_t.shape",dec_mean_t.shape)
                    #print(dec_mean_t.detach().cpu().numpy())
                    dec_std_t = self.dec_std_prior(dec_t)
                #print(dec_mean_t.detach().cpu().numpy().shape)
