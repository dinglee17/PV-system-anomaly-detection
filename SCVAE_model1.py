...
class Normal_VRNN(nn.Module):
    ...
    def reucrrence(self, x_t, y_t, h, h2, recording=True): #prior_flag 的含义是先验推导还是后验推导,如果为1的话表明先验不使用标签，如果为0的话表明为后验，使用标签,如果为2的话采用两种模式混合的方式
        ...
        z_t_prior_predict = self._reparameterized_sample(predict_mean_t, predict_std_t)
        ...
        phi_z_t_predict= self.phi_z(z_t_prior_predict)
        ...
        dec_t_prior = self.dec(torch.cat([phi_x_t,phi_z_t_prior, h],1)) # 考虑的是xt,zt条件下的y值
        ...
        dec_t_predict = self.dec(torch.cat([phi_x_t,phi_z_t_predict, h2],1)) # 考虑的是xt,zt条件下的y值
